...
export class PasswordResetService implements IPasswordResetService {
  constructor(
    ...
    private tokenLifecycleRepository: ITokenLifecycleRepository,
  ...
  async forgotPassword(email: string): Promise<void> {
    ...
    await this.tokenLifecycleRepository.issuePasswordResetToken({
      hashedToken,
      userId: user.id,
  ...
  async resetPassword(input: ResetPasswordDTO): Promise<void> {
    ...
    const userId = await this.tokenLifecycleRepository.getPasswordResetUserId(hashedToken);
    const isTokenValid = await this.tokenLifecycleRepository.isPasswordResetTokenValid(hashedToken);

    if (!userId || !isTokenValid) throw new AppError(AUTH_MESSAGES.INVALID_TOKEN, STATUS_CODES.BAD_REQUEST);
    ...
    await this.tokenLifecycleRepository.consumePasswordResetToken(hashedToken);
    logger.info(`Password reset successful for userId: ${userId}`);
  }

  async validateResetToken(token: string): Promise<boolean> {
    const hashedToken = crypto.createHash('sha256').update(token).digest('hex');
    return this.tokenLifecycleRepository.isPasswordResetTokenValid(hashedToken);
  }
}
